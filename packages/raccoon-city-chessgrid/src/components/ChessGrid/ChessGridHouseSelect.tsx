...
function GroupHeading(props) {
  const imageUrl = props.apartmentComplexImage
    ? props.apartmentComplexImage.downloadUrl
    : houseDefaultImage;
  return (
    <StyledHeading>
...
const Group = props => {
  return (
    <components.Group
      {...props}
      headingProps={{
        apartmentComplexImage: props.data.apartmentComplexImage,
        ...props.headingProps
      }}
    />
  );
...
function CustomOption(props: { data: House } & any) {
  ...
  const imageUrl = data.images.CHESS_GRID
    ? data.images.CHESS_GRID.downloadUrl
    : houseDefaultImage;
  return (
    <StyledList
...
export function ChessGridHouseSelect({ onChange }: ChessGridHouseSelectProps) {
  ...
  const options = data.getApartmentComplexesByDeveloper.map(
    apartmentComplex => {
      return {
